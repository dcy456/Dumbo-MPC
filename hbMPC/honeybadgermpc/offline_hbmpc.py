...
from honeybadgermpc.time_out import decide
...
async def generate_triples(n, t, k, my_id, _send, _recv, field):
    ...
    # logger.info(f'[{my_id}] Generate triples complete')
    subscribe_recv_task.cancel()
...
async def _run(peers, pbk, pvk, n, t, k, my_id, max_timeout = 0.5):
    ...
    async with ProcessProgramRunner(peers, n, t, my_id) as runner:
        ...
        logger.info(f'[{my_id}] start hbMPC')

        round = 1
        while True:             
            send, recv = runner.get_send_recv(f'round_{round}')
            ...
            logger.info(f'[{my_id}] finished {round}-th round hbMPC in {triple_time}, triple number: {k}')
        ...
        logger.info(f'[{my_id}] Finished in {end_time - start_time} s')
...
if __name__ == "__main__":
    ...
    global logger 
    logfile = f'./log/logs-{HbmpcConfig.my_id}.log'
