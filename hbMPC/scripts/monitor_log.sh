...
NUM_NODES=$1
LOG_DIR="./log"  # Adjust the path to your logs directory if needed
...
while true; do
    ...
    for ID in $(seq 0 $((NUM_NODES - 1))); do
        ...
        # Check if the 'AsyRanTriGen finished!' or 'finished in' keyword appears in the log file
        if grep -q -E "Finished in" "$LOG_FILE"; then
