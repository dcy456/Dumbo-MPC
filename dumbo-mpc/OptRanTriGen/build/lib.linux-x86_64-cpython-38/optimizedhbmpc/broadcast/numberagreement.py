...
from optimizedhbmpc.exceptions import RedundantMessageError, AbandonedNodeError
...
def handle_conf_messages(*, sender, message, conf_values, pid, bv_signal):
    ...
    if len(v_list) == 1:
        v_mod = (v_list[0] % 2, )
    if len(v_list) == 2:
        v_mod = (v_list[0] % 2, v_list[1] % 2)

    if sender in conf_values[r][v_mod]:
        logger.warning(
    ...
    conf_values[r][v_mod].add(sender)
    logger.debug(
...
async def wait_for_conf_values(
    *,
    ...
    while True:
        ...
        if index ==1:
            index_inv = 0
        if len(conf_values[epoch][(index,)]) >= n - f:
            return set((input_msg,))
        if len(conf_values[epoch][(index_inv,)]) >= n - f:
            temp = bin_values[epoch]
            return set(temp,)
        if len(bin_values[epoch]) == 2:
            bin_values_mod = (list(bin_values[epoch])[0] % 2, list(bin_values[epoch])[1] % 2)
            if (
                sum(
                    len(senders)
                    for conf_value, senders in conf_values[epoch].items()
                    if senders and set(conf_value).issubset(bin_values_mod)
                )
                >= n - f
            ):
                print("set(sorted(bin_values[epoch]))", set(sorted(bin_values[epoch])))
                return set(sorted(bin_values[epoch]))
        ...
        while True:  # not finished[pid]:
            ...
            if msg[0] == "EST":
                ...
                if len(est_values[r][v]) >= 2 * f + 1:
                    logger.debug(
                        f"[{pid}] add v = {vi} to bin_value[{r}] = {bin_values[r]}",
                        extra={"nodeid": pid, "epoch": r},
                    )
                    bin_values[r].add(vi)
                    logger.debug(
    ...
    try:
        ...
        while True:  # Unbounded number of rounds
            ...
            while True:
                ...
                if len(aux_values[r][index]) >= n - f:
                    values = set((vi,))
                    # print('[sid:%s] [pid:%d] VALUES 1 %d' % (sid, pid, r))
                    break
                if len(aux_values[r][index_inv]) >= n - f:
                    temp = bin_values[r]
                    values = set(temp,)
                    # print('[sid:%s] [pid:%d] VALUdES 0 %d' % (sid, pid, r))
                    break
                if sum(len(aux_values[r][v % 2]) for v in bin_values[r]) >= n - f:
                    values = set(sorted(bin_values[r]))
                    # print("values", values)
            ...
            # print("conf_sent[r]", conf_sent[r])
            # print("values", values)
            # print("tuple(v % 2 for v in values)", tuple(sorted(v % 2 for v in values)))
            if not conf_sent[r][tuple(sorted(v % 2 for v in values))]:
