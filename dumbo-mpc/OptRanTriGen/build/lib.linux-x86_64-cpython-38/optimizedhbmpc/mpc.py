from optimizedhbmpc.progs.mixins.dataflow import (
    Share,
    ShareArray,
    ShareFuture,
    GFElementFuture,
...
from .exceptions import OptimizedhbmpcError
    ...
    def open_share(self, share):
        ...
        def cb(r):
            ...
            if p is None:
                ...
                res.set_exception(
                    OptimizedhbmpcError(f"Failed to open share with id {shareid}!")
                )
            else:
    ...
    def open_share_array(self, sharearray):
        ...
        def cb(r):
            ...
            if elements is None:
                ...
                res.set_exception(OptimizedhbmpcError("Batch reconstruction failed!"))
            else:
        ...
        reconstructed = asyncio.create_task(
            batch_reconstruct(
                ...
                debug=True,
                degree=degree,
                confirmed_errors = self.block_list,
            )
        )
    ...
    async def _run(self):
        ...
        if bgtask.done():
            ...
            if bg_exception is None:
                raise OptimizedhbmpcError("background task finished before prog!")
            else:
                raise bg_exception
...
async def test_batchopening(context):
    ...
    ys = [context.preproc.get_zero(context) + context.Share(i) for i in range(100)]
    zs = [xs[i] + ys[i] for i in range(100)]
    zs = context.ShareArray(zs)
    zs_ = await zs.open()
    for i, x in enumerate(zs_):
        assert x.value == 2* i
    # xs = context.ShareArray(xs)
    # xs_ = await xs.open()
    # for i, x in enumerate(xs_):
    #     assert x.value == 2* i
    print(f"[%d] Finished batch opening % {(context.myid,)}")
...
async def test_prog1(context):
    ...
    assert x_ * y_ == xy_
    print(f"[{context.myid}] Finished {x_}, {y_}, {xy_}")
...
async def test_prog2(context):
    ...
    for share in shares[:100]:
        s = await share.open()
        assert s == 0
    print(f"[%d] Finished {(context.myid,)}")
    ...
    for s in await arr.open():
        assert s == 0, s
    print(f"Finished batch % {(context.myid,)}")
