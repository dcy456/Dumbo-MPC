...
async def _recv(ba_task, n, receive,):
...
async def _run(peers, pbk, pvk, n, t, k, my_id):
    ...
    async with ProcessProgramRunner(peers, n, t, my_id) as runner:
        ...
        for i in range(round):
            send, recv = runner.get_send_recv(f'{i}_0')
            triples = await generate_triples(n, t, k, my_id, send, recv, field)
            print(f'{my_id} finishs {i}-th round')
            if i == 2:
                inputq.put_nowait(random.randint(3, 4))
                await ba_task
                ba_value = await outputq.get()
                decision = True
                print(f'NODEID {my_id} BA VALUE: {ba_value}')
                crecv_task.cancel() 
            if decision:
                break
        end_time = time.time()
        print(f'{my_id} Finished in {end_time - start_time} s')
        print(f'{my_id} totally generate {ba_value * k} beaver triples')
