...
async def _run(peers, pbk, pvk, n, t, k, my_id, max_timeout = 0.5):
    ...
    sid_c = "sid_coin"
    sid_ba = "sid_ba"
    sid_decide = "sid_decide"
    async with ProcessProgramRunner(peers, n, t, my_id) as runner:
        
        # start ABA
        send_c, recv_c = runner.get_send_recv(sid_c)
        ...
        coin, crecv_task = await shared_coin(
            sid_c, my_id, n, t, pbk, pvk, bcast_c, recv_c
        )

        inputq = asyncio.Queue()
        outputq = asyncio.Queue()

        send_ba, recv_ba = runner.get_send_recv(sid_ba)
        ...
        outputdeq = asyncio.Queue()

        send_decide, recv_decide = runner.get_send_recv(sid_decide)

        def bcast_de(o):
            for i in range(n):
                send_decide(i, o)

        de_task = decide(n, outputdeq.put_nowait, recv_decide)
        ...
        round = 0
        while True:
            if round == 10 and my_id ==0:
            # if round >3 and triple_time > max_timeout:
                bcast_de("time_out")
                
            send, recv = runner.get_send_recv(f'{round}_0')
            ...
            print(f'[{my_id}] finished {round}-th round opt. triple generation in {triple_time}')
      
            if not outputdeq.empty():
                msg = await outputdeq.get()
                if msg == "time_out":
                    acs_starttime = time.time()
                    print(f'[{my_id}] Start switch to roubust triple generation')
                    inputq.put_nowait(round)
                    await ba_task
                    ba_value = await outputq.get()
                    acs_endtime = time.time()
                    decision = True
                    print(f'[{my_id}] Agree on round {ba_value}')
                    print(f'[{my_id}] Finish switch in {acs_endtime - acs_starttime} s')
                    crecv_task.cancel() 
        ...
        if asyncio.get_event_loop().is_running():
            _thread_de.cancel()
        print(f'[{my_id}] Finished in {end_time - start_time} s')
        print(f'[{my_id}] totally generate {(ba_value +1) * k} beaver triples')

        import subprocess
        shell_script_path = "./scripts/run_beaver.sh"

        params = [shell_script_path, str(n) , str(20) , str(my_id)]
        logfile = open(f'./log/logs-{my_id}.log', 'w')
        subprocess.run(params, check=True ,stdout=logfile )
