...
from optimizedhbmpc.config import HbmpcConfig, ConfigVars
from optimizedhbmpc.utils.misc import wrap_send, subscribe_recv
from optimizedhbmpc.utils.misc import print_exception_callback
...
class NodeCommunicator(object):
    ...
    async def _recv_loop(self, router):
        while True:
            ...
            # print(f"[RECV] FROM: %s, MSG: %s,", sender_id, msg)
    ...
    async def _process_node_messages(self, node_id, node_msg_queue, send_to_node):
        while True:
            ...
            # print("[SEND] TO: %d, MSG_TYPE: %s", node_id, msg[1][0])
            msg_type = msg[1][0][0:3]
...
async def test_mpc_programs(peers, n, t, my_id):
    from optimizedhbmpc.mpc import test_prog1, test_prog2, test_batchopening
    from optimizedhbmpc.preprocessing import PreProcessedElements
    from optimizedhbmpc.preprocessing import wait_for_preprocessing, preprocessing_done
