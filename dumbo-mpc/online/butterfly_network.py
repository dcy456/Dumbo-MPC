...
async def iterated_butterfly_network(ctx, inputs, k):
    ...
    for _ in range(num_iterations):
        ...
        while stride < k:
            ...
            logger.info(f"[ButterflyNetwork-{iteration}]: {time()-stime}")
            iteration += 1
    return inputs
...
async def butterfly_network_helper(ctx, **kwargs):
    ...
    starttime = time()
    logger.info(f"[{ctx.myid}] Running permutation network.")
    ...
    if shuffled is not None:
        ...
        # print((f"[{ctx.myid}] {opened_values}"))
        logger.debug(f"[{ctx.myid}] {opened_values}")
        endtime = time()
        logger.info(f"[{ctx.myid}] finished in {endtime - starttime} s")
...
async def _run(peers, n, t, my_id):
    ...
    from optimizedhbmpc.progs.mixins.share_arithmetic import (
        MixinConstants,
