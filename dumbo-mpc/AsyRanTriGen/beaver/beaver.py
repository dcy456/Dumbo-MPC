...
class BEAVER:
    ...
    async def run_beaver(self, node_communicator):
        ...
        logger.info(f"[{self.my_id}] [random share generation] Starting ACS where node {leader} is set as leader ")
        logger.info(f"[{self.my_id}] [random share generation] The proposal is {key_proposal}")
        ...
        logger.info(f"[{self.my_id}] [random share generation] The ACS set is {acsset}") 

        logger.info(f"[{self.my_id}] [random share generation] Starting compute random shares") 
        randomshares_proofs = self.genrandomshare(acsset, acss_outputs)
        acss_outputs = [None]
        logger.info(f"[{self.my_id}] [random share generation] Obtaining total {(self.t + 1) * self.batchsize} random shares!")

        
        logger.info(f"[{self.my_id}] Starting to generate triples over random shares")
        logger.info(f"[{self.my_id}] [beaver triple generation] Starting to re-share product of random shares") 
        reduction_outputs = {}
        ...
        leader = 2
        logger.info(f"[{self.my_id}] [beaver triple generation] Starting to ACS where {leader} is set as leader ")

        logger.info(f"[{self.my_id}] [beaver triple generation] The proposal of node {self.my_id} is {reduction_proposal}")                
        acs = OptimalCommonSet(
            acstag_beaver,
            self.my_id,
            self.n,
        ...
        logger.info(f"[{self.my_id}] [beaver triple generation] The ACS set is {acsset_beaver}") 
        
        # for item in acsset_beaver:
        #     if item not in reduction_outputs.keys():
        #         await reduction_signal.wait()
        #         reduction_signal.clear()
